...
Caf.defMod(module, () => {
  return Caf.importInvoke(
    ...
    (
      ...
      return (S3P = Caf.defClass(
        class S3P extends Object {},
        function (S3P, classSuper, instanceSuper) {
          this.list = (options) => {
            let list;
            return S3C.each(
              merge(options, {
            ...
            return S3C.each(
              merge(options, {
                getProgress: () =>
                  compactFlatten([
                    `totalSize: ${Caf.toString(
                      colors.green(
                        `${Caf.toString(humanByteSize(summary.size))}`
                      )
                    )}`,
                ...
                map: ({ Size, Key }) => {
                  ...
                  summary.size += Size;
                  if (
                    (temp2 = Size >= summary.maxSize) != null ? temp2 : Size
                  ) {
                    summary.maxSize = Size;
                    summary.maxSizeKey = Key;
                  }
                  if (
                    (temp3 = Size <= summary.minSize) != null ? temp3 : Size
                  ) {
                ...
                getProgress: (duration) =>
                  compactFlatten([
                    ...
                    `copyWorkers: ${Caf.toString(
                      colors.green(
                        `${Caf.toString(
                          copyPwp.activeWorkers
                        )} + ${Caf.toString(largeCopyPwp.activeWorkers)}`
                      )
                    )}`,
                    0 < copyPwp.queueSize + largeCopyPwp.queueSize
                      ? `copyQueue: ${Caf.toString(
                          colors.green(
                            `${Caf.toString(
                              copyPwp.queueSize
                            )} + ${Caf.toString(largeCopyPwp.queueSize)}`
                          )
                        )}`
                      : undefined,
